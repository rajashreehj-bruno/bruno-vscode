...
interface CONNECTION_STATUSProps {
...
const useWsConnectionStatus = (requestId: string): [string, React.Dispatch<React.SetStateAction<string>>] => {
  const [connectionStatus, setConnectionStatus] = useState(CONNECTION_STATUS.DISCONNECTED);
  useEffect(() => {
    const checkConnectionStatus = async () => {
      const result = await getWsConnectionStatus(requestId) as { status?: string } | null;
...
const WsQueryUrl = ({
  ...
  const handleDisconnect = async (e?: React.MouseEvent, notify = true) => {
    e?.stopPropagation();
  ...
  const handleRunClick = async (e: any) => {
    e.stopPropagation();
    if (!url) {
