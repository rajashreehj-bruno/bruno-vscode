...
interface TriggerButtonProps {
  method: string;
  methodSpanRef?: React.RefObject<HTMLSpanElement>;
  showCaret?: boolean;
}
...
const HttpMethodSelector = ({
  ...
  const methodSpanRef = useRef<HTMLSpanElement>(null);
  const [previousMethodWidth, setPreviousMethodWidth] = useState<number | null>(null);

  const { theme } = useTheme();
  const methodColor = useMemo(() => {
    const methods = theme.request.methods as Record<string, string>;
    return methods[method.toLocaleLowerCase()] || theme.request.methods.get;
  }, [method, theme]);
  ...
  const handleMethodSelect = useCallback((verb: string) => {
    onMethodSelect(verb);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    switch (e.key) {
      ...
      case KEY.ENTER: {
        onMethodSelect((e.target as HTMLInputElement).value ? (e.target as HTMLInputElement).value.toUpperCase() : DEFAULT_METHOD);
        setIsCustomMode(false);
        blurInput();
        return;
      }
  ...
  const menuItems = useMemo(() => {
    const items: { id: string; label: string; onClick: () => void }[] = STANDARD_METHODS.map((verb) => ({
      id: verb.toLowerCase(),
      label: verb,
