...
const QueryParams = ({
  ...
  const queryParams = params.filter((param: any) => param.type === 'query');
  const pathParams = params.filter((param: any) => param.type === 'path');
  ...
  const handleQueryParamsChange = useCallback((updatedParams: any) => {
    const paramsWithType = updatedParams.map((p: any) => ({
      ...p,
      type: 'query'
    }));
  ...
  const handlePathParamChange = useCallback((rowUid: any, key: any, value: any) => {
    const pathParam = pathParams.find((p: any) => p.uid === rowUid);
    if (pathParam) {
    ...
    },
    {
      ...
      render: ({
        ...
        <MultiLineEditor
          ...
          theme={storedTheme}
          onSave={onSave}
          onChange={(newValue: any) => handlePathParamChange(row.uid, 'value', newValue)}
          onRun={handleRun}
          collection={collection}
          item={item}
        />
