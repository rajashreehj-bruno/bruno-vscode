 ...
 */
const posixify = (str: string): string => {
 ...
 * @returns The relative path from `fromPath` to `toPath`, `"."` if both are the same,
 ...
 *
 * @param basePath - The base path to resolve against (e.g., "/users/john/projects")
 * @param relativePath - The relative path to resolve (e.g., "../docs/file.txt")
 * @returns The basename of the resolved path, or empty string if relativePath is falsy
 *
 * @example
 ...
 *  → "projects"
 */
const getBasename = (basePath: string, relativePath: string): string => {
  if (!relativePath) {
    return '';
  }
